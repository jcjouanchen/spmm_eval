...
__global__ void printGlobalBSRBlock2(const uchar *bsrval, const int blocksize, const int nblocks)
{
    printf("--- global bsr 2 block (bitmap) --- \n");
...
__global__ void countBlockStats(int *stats, const uchar *bsrval, const int blocksize, const int nblocks)
{
    // printf("--- global bsr 4 block (bitmap) --- \n");
    for (int b = 0; b < nblocks; b++)
    {
        ...
        for (int j = 0; j < blocksize; j++)
        {
            ...
            // }
            // printf("\n");
            sum += __popc(bsrval[b * blocksize + j]);
        }
        stats[sum-1] += 1;
    }
}
...
__global__ void countBlockStats2(int *stats, const float *bsrval, const int blocksize, const int nblocks)
{
    // printf("TempBSRVal: \n");
    for (int i = 0; i < nblocks; i++)
    {
        ...
        for (int j = 0; j < blocksize; j++)
        {
            for (int k = 0; k < blocksize; k++)
            {
                sum += ((bsrval[i * blocksize * blocksize + j * blocksize + k]) > 0 ? 1:0);
                // printf(bsrval[i * blocksize * blocksize + j * blocksize + k] > 0 ? "1" : "0");
            }
            // printf("\n");
        }
